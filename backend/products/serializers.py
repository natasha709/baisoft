# Product Data Serializers
# This module defines how product data is converted between JSON (API) and Python objects (models).
# It handles different serialization needs for public browsing, product management, and creation.
#
# Key Features:
# - Public product serializer for customer browsing (limited fields)
# - Full product serializer for business management (includes audit trail)
# - Creation serializer with business validation
# - Price validation and business rule enforcement
# - Audit trail fields (who created, who approved, when)
...
class PublicProductSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'price',        # Product price
            'business_name', # Business name (historical snapshot)
        ]
...
class ProductCreateSerializer(serializers.ModelSerializer):
    ...
    business = serializers.PrimaryKeyRelatedField(
        queryset=Product.objects.all(),  # This queryset is overridden in the view
        required=False,  # Optional - defaults to user's business
        allow_null=True  # Allow null for default business assignment
    )
    ...
    class Meta:
        ...
        fields = [
            'name',         # Product name (required)
            'description',  # Product description (required)
            'price',        # Product price (required, validated)
            'business'      # Business assignment (optional)
        ]
