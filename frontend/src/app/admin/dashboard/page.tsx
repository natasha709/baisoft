...
export default function AdminDashboard() {
    ...
    const fetchStats = async () => {
        try {
            ...
            // Extract activities: Approval actions and new product additions
            const activities = products
                .slice(0, 5)
                .map((product: any) => ({
                    id: product.id,
                    type: product.status === 'approved' ? 'approval' : 'creation',
                    time: product.updated_at || product.created_at,
                    message: product.status === 'approved'
                        ? `Approved: ${product.name}`
                        : `New Draft: ${product.name}`,
                    business: product.business_name_snapshot || product.business_name || 'System',
                    user: product.approved_by_email || product.created_by_email || 'N/A'
                }));
            ...
            <div className="flex-1 overflow-auto">
                <div className="p-8 max-w-7xl">
                    ...
                    <div className="mt-12 grid grid-cols-1 lg:grid-cols-3 gap-8">
                        <div className="lg:col-span-2 bg-white rounded-[2rem] p-8 border border-gray-100 shadow-xl shadow-gray-200/50 flex flex-col">
                            ...
                            <div className="flex-1 space-y-4">
                                {recentActivity.length > 0 ? (
                                    recentActivity.map((activity, idx) => (
                                        <div key={idx} className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl border border-gray-100 hover:border-blue-200 transition-colors group">
                                            <div className="flex items-center gap-4">
                                                <div className={`p-2.5 rounded-xl ${activity.type === 'approval' ? 'bg-emerald-100 text-emerald-600' : 'bg-blue-100 text-blue-600'}`}>
                                                    {activity.type === 'approval' ? <CheckCircle2 className="w-5 h-5" /> : <Package className="w-5 h-5" />}
                                                </div>
                                                <div>
                                                    <p className="font-bold text-gray-900 group-hover:text-blue-600 transition-colors uppercase text-xs tracking-tight">{activity.message}</p>
                                                    <p className="text-xs text-gray-500 mt-0.5">By {activity.business}</p>
                                                </div>
                                            </div>
                                            <div className="text-right">
                                                <p className="text-xs font-bold text-gray-400 flex items-center gap-1.5 justify-end">
                                    ...
                                    <div className="h-48 flex items-center justify-center text-gray-400 font-medium italic border-2 border-dashed border-gray-100 rounded-2xl">
